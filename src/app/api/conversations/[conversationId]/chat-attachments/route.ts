...
export async function POST(request: Request, { params }: { params: Promise<{ conversationId: string }> }) {
  try {
    ...
    const existingPending = await prisma.chatAttachment.count({
      where: { conversationId, uploaderUserId: auth.authUserId, chatMessageId: null },
    });
    if (existingPending + files.length > MAX_ATTACHMENTS) {
      return NextResponse.json(
        { error: `Maximum ${MAX_ATTACHMENTS} pending attachment files allowed.` },
        { status: 400 },
      );
    }
    ...
    console.error("POST /api/conversations/[conversationId]/chat-attachments failed", error);
